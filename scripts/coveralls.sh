...
declare -r xctoolVars=$(xctool -workspace ContentfulSDK.xcworkspace -scheme ContentfulDeliveryAPI -configuration Debug -sdk iphonesimulator -arch i386 -showBuildSettings | egrep '(BUILT_PRODUCTS_DIR)|(CURRENT_ARCH)|(OBJECT_FILE_DIR_normal)|(SRCROOT)|(OBJROOT)' |  egrep -v 'Pods')
while read line; do
...
generateGcov()
{
	...
	do
		gcov-4.2 "${file}" -o "${gcov_dir}" >/dev/null 2>&1
	done
	cd - >/dev/null
}
